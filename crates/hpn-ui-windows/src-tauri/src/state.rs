...
// ConnectionStatus::log_change() removed — use tracing::info! directly for status transitions.
...
mod tests {
    ...
    fn test_update_uptime() {
        let mut state = AppState::new();
        // No connected_at -> uptime stays 0
        state.update_uptime();
        assert_eq!(state.stats.uptime, 0);
        // With connected_at -> uptime >= 0
        state.connected_at = Some(Instant::now());
        std::thread::sleep(std::time::Duration::from_millis(10));
        state.update_uptime();
        // Should be at least 0 seconds (timer precision)
    ...
    fn test_sanitize_status_resets_stale() {
        ...
        // No shutdown_tx means it's stale
    ...
    fn test_sanitize_status_keeps_connected() {
        ...
        state.sanitize_status();
        // Connected is not a "stale" state
        assert_eq!(state.status, ConnectionStatus::Connected);
    }
}

...
const isValidCIDR = (cidr: string): boolean => {
  ...
  // exclude the entire IPv4 internet, defeating the tunnel. The Swift
  // validator (PacketTunnelProvider.swift::routesFromCidrs) silently
  // drops /0; we reject it in the UI so the user gets immediate
  // feedback instead of a confusing "the route did nothing" symptom.
  const ipv4Regex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;
...
export const ProfileEditor: React.FC<ProfileEditorProps> = ({ mode, initialValues, onCancel, onSubmit, t }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      ...
      verified: initialValues?.verified || false, // Preserve or default
      securityLevel,
      requiresAuth,
